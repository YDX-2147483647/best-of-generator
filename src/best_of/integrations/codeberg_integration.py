...
class CodebergIntegration(BaseIntegration):
    @property
    def name(self) -> str:
        return "codeberg"

    def update_project_info(self, project_info: Dict) -> None:
        if not project_info.codeberg_id:
            return

        try:
            params = Dict()
            response = requests.get(
                f"https://codeberg.org/api/v1/repos/{project_info.codeberg_id}",
                params=params,
            )
            ...
            log.info(
                f"Failed to request the repo {project_info.codeberg_id} via Codeberg API ",
                exc_info=ex,
            )
            return

        if not project_info.codeberg_url:
            project_info.codeberg_url = repo_info.html_url

        if not project_info.homepage:
            project_info.homepage = repo_info.website or project_info.codeberg_url
        ...
        if repo_info.stars_count:
            stars_count = int(repo_info.stars_count)
    ...
    def generate_md_details(self, project: Dict, configuration: Dict) -> str:
        # Only show if codeberg url is set
        if not project.codeberg_id or not project.codeberg_url:
            return ""
        ...
        details_md = f"- [Codeberg]({project.codeberg_url}) {metrics_md}{separator}\n"

        if configuration.generate_install_hints:
            details_md += f"\n\t```\n\tgit clone {project.codeberg_url}\n\t```\n"
